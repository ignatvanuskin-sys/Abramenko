...
# Button text uses plain Unicode characters, not E.* premium emojis (which don't render in buttons)
...
def time_slots_kb(slots: dict[str, str], back: str = "back_to_date") -> InlineKeyboardMarkup:
    ...
    if not free_slots:
        return InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="⏳ В лист ожидания", callback_data="go_to_waitlist")],
...
def confirm_cancel_kb(booking_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="↩️ Нет, вернуться", callback_data=f"booking_detail:{booking_id}"),
        ],
    ])
...
def admin_services_kb() -> InlineKeyboardMarkup:
    ...
    for i in range(0, len(service_items), 2):
        ...
        buttons.append(row)
    buttons.append([
        InlineKeyboardButton(text="➕ Добавить", callback_data="admin_add_service"),
        InlineKeyboardButton(text="← Назад", callback_data="admin"),
    ])
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def admin_service_detail_kb(name: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✏️ Редактировать", callback_data=_safe_cb("admin_edit_service:", name)),
...
def admin_unavailable_kb(periods: list[dict]) -> InlineKeyboardMarkup:
    ...
    for period in periods[:10]:
        label = f"🗑 #{period['id']} — {period['date']} {period['start_time']}-{period['end_time']}"
