...
LEADS = {
    "model": ("Стать моделью", [("name", "Ваше имя?"), ("phone", "Телефон +7XXXXXXXXXX?"), ("service", "Какая услуга интересует?"), ("portfolio", "Ссылка на портфолио или «нет»?"), ("branch", "Какой филиал? Abramenko Studio (Букетова, 61) или Мадам (Жамбыла, 127)?")]),
    "vacancy": ("Вакансия", [("name", "Ваше имя?"), ("experience", "Расскажите об опыте."), ("portfolio", "Ссылка на портфолио или «нет»?"), ("phone", "Телефон +7XXXXXXXXXX?"), ("branch", "Какой филиал? Abramenko Studio (Букетова, 61) или Мадам (Жамбыла, 127)?")]),
...
async def lead_field(m: Message, state: FSMContext):
    ...
    if field == "branch" and value.lower() not in {x.lower() for x in BRANCHES}:
        return await m.answer("Выберите филиал из списка: Abramenko Studio (Букетова, 61) или Мадам (Жамбыла, 127).")
